...
def get_projname(year):
    ...
    if year==2014 or year==2013 or year==2012:
    	...
    	while len(hh)>0:
    		...
    		hh.remove(hh[0])			
    	#nwnw=[]
    	#for thing in tup1lst:
    		#nn=thing+'/r'
    		#nwnw.append()
    	#nww2=[]
    	#for thing in tup2lst:
    		#tt=thing+'/t'
    		#nwnw.append(tt)
    	#nwnw=nww+nww2
    	hh=tup1lst
...
# print(ww)
		...
		# for thin in tup[1]:
		# 	if '\r' not in thin and '\t' not in thin:
		# 		abc=thin
		# 	if '\r' in thin:
		# 		m=thin.strip('	')
		# 		j=m.split(',')
...
def plot_institutions(year):
	...
	for place in bb:
		...
		parameters['key']= "AIzaSyCAZ9u_0iNyDCo9GTfysqMQqKLAIvmu4k4"
...
# # 	#insertion=(listt[0])
# # insert='''INSERT INTO AEFProjects (Director)'''
# # params=()
# # for listt in k:
# # 	params=params+(listt[0],)
# 	# insert='UPDATE AEFProjects SET Director='
# 	# insert+=str(listt[0])
# #insert2='UPDATE Bars SET BroadBeanOriginId=(SELECT Id FROM Countries WHERE Bars.BroadBeanOrigin=Countries.EnglishName)'
